...
func test_shape_ids_resolve_to_catalogued_geometry() -> void:
	...
	var diagonal_shape: int = CellShapeCatalog.shape_code_for("DG")
	...
	assert_eq(diagonal_shape, CellShapeCatalog.DIAGONAL_SHAPE)
	...
	assert_eq(catalog.polygon_for(diagonal_shape).size(), 4)
