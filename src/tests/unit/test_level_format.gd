...
func test_authored_map_loads_with_repeated_row_tokens() -> void:
	...
	assert_eq(LevelLayer.cell_background(roof_code), 1)
	var water_code: int = level.layers[0].cell_code_at(0, 40)
	assert_eq(LevelLayer.cell_background(water_code), 3)
	var circle_code: int = level.layers[0].cell_code_at(8, 45)
	assert_eq(LevelLayer.cell_shape(circle_code), LevelLayer.CIRCLE_SHAPE)
	var right_triangle_code: int = level.layers[0].cell_code_at(8, 46)
	assert_eq(LevelLayer.cell_shape(right_triangle_code), LevelLayer.RIGHT_TRIANGLE_SHAPE)
	var half_cell_code: int = level.layers[0].cell_code_at(8, 47)
