...
On systems where `python` is Python 2, use `python3`. Stages run in order:

| Stage | What it proves |
| --- | --- |
| `integrity` | The gate itself is unmodified. `src/project.godot` changes are noted, not failed |
...
| `conformance` | code vs `proposal.json` when `status` is `approved`, including each file's declared `create`/`modify` against the tree at `baseline_sha`, at the depth `involvement` implies. A `draft` is reported as awaiting review and not diffed. **Fails** if the proposal has structure but no `experience` block with `player_does` and `feels_like`, or if a `design_refs` entry points at a file that does not exist. Warns when there are no `design_refs` at all | report the deviation, append to `revisions`, set `status` back to `draft`, re-approve. For a missing contract, write `experience` before touching structure |
...
```
python check.py --only typecheck          # repeatable, or comma-separated
python check.py --fast                    # skip import when only .gd changed
python check.py --fix-format              # scoped format fix
python check.py --accept-gate-changes     # HUMAN ONLY, re-baselines .gate.sha256
```
...
you can edit is not a gate.

**`format` fails** — `python check.py --fix-format`. Never bare `gdformat .`; it is
unscoped and reformats `src/addons/`, which is third-party code.
...
on newly added GDScript syntax, so a crash is reported as SKIP rather than FAIL.

**`sanitise` fails** — `python sanitise.py --write` applies the mechanical fixes
...
**`typecheck` fails** — fix the code. Never lower a warning in `src/project.godot`.
...
repo that passes: **`docs/GDSCRIPT.md`** (generated — regenerate with
`python tools/gen_gdscript_doc.py --write`).
...
server running via a text scan. If you start it, `python tools/gdls.py stop` before
running the gate — a live Godot instance holds `.godot/`, which the gate writes to.
...
**`shape` skips, never fails**, when fields are still `unknown`. It is a prompt to have
the onboarding conversation, not a defect. It fails only on unparseable JSON.
...
conversion into `src/scripts/data/`, return a typed object, and change the logic signature to
accept it. See `GDSCRIPT.md`. A deliberate exception takes a trailing `# gate:allow`.

**`arch` fails** two distinct ways. A **stale graph** means dependencies changed without
regeneration: run `python arch.py --write` and commit. A **boundary violation** means a
dependency `arch.rules.json` does not permit: fix the code, do not widen the rule
without asking.

**`assets` fails** — run `python check.py --only import`, then commit the generated
...
**`gut` fails** — read `.checklogs/gut.log`. Zero collected tests is a FAIL, not a pass.
...
- **`arch.rules.json` and `src/project.godot` are advisory, not pinned.** Adding a module is a
  real architecture change that needs a real edit, and Godot rewrites `src/project.godot`
...
Drop source files in and run `python check.py --only import`. Godot generates the
`.import` sidecar; commit it alongside the source. Never hand-edit one.

Import defaults live in `src/project.godot` under `[importer_defaults]`, set once by
`python bootstrap.py --import-profile <name>`. **Do not change them casually.** Godot
...
```
python tools/gddoc.py --build             # once, dumps the class reference
python tools/gddoc.py Color               # class summary
python tools/gddoc.py Color.from_string   # exact signature
...
not `hands-off`, `module` or `function`. A typo there would silently drop the human
